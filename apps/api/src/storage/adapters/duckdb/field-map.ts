...
/**
 * Logical→physical column mapping (docs/architecture.md §6).
 *
 * The compiler hands us LOGICAL field identifiers (camelCase contract vocabulary
 * such as "latencyMs", "model", "durationMs"). This adapter owns the translation
 * to physical snake_case SQL columns / expressions, per source table.
 *
 * `events` is the wide source-of-truth table; `runs`/`traces` are SQL views that
 * derive rollups from it (see schema.ts). Each source therefore exposes a
 * different set of logical fields, so we keep one map per source.
 */

/**
 * Logical fields that resolve to a SQL EXPRESSION rather than a bare column.
 * `totalTokens` only exists at the event grain as (input + output); at the
 * rollup grain it is materialised as a real `total_tokens` column in the view.
 */
