...
import { ClickHouseEventStore } from "../../src/storage/index.js";

/**
 * Runs against the LIVE local ClickHouse (docker compose). Uses a DEDICATED
 * database `ata_test` so it never touches app data. Mirrors duckdb.test.ts for
 * apples-to-apples contract parity (same sample batch, same assertions).
 */
...
/**
 * Two traces, mirroring the fixture shapes (identical to duckdb.test.ts):
...
// CH round-trips need a generous timeout (network + insert flush).
const TIMEOUT = 30_000;

let store: ClickHouseEventStore;

beforeAll(async () => {
  store = new ClickHouseEventStore(CH);
  await store.init();
}, TIMEOUT);

beforeEach(async () => {
  // Views are stateless; only the base table holds state. TRUNCATE between tests.
  const admin = createClient(CH);
  try {
    await admin.command({ query: `TRUNCATE TABLE IF EXISTS ${CH.database}.events` });
  } finally {
    await admin.close();
  }
}, TIMEOUT);
...
afterAll(async () => {
  const admin = createClient(CH);
  try {
    await admin.command({ query: `DROP DATABASE IF EXISTS ${CH.database}` });
  } finally {
    await admin.close();
  }
  await store.close();
...
describe("ClickHouseEventStore", () => {
  it(
    "inserts a batch of events across runs/traces",
    async () => {
  ...
  it(
    "is idempotent by eventId (dedup-before-insert)",
    async () => {
      ...
      // Row count unchanged: a count(*) aggregate over all events (no FINAL needed).
  ...
  it(
    "aggregates count by toolName (event grain)",
    async () => {
      ...
      expect(res.engine).toBe("clickhouse");
  ...
  it(
    "aggregates avg latencyMs by model (event grain)",
    async () => {
      await store.insertBatch(sampleEvents());
      const query: CompiledQuery = {
        source: "events",
  ...
  it(
    "aggregates a latency quantile (p95) over events",
    async () => {
      await store.insertBatch(sampleEvents());
      const query: CompiledQuery = {
        source: "events",
  ...
  it(
    "aggregates avg costUsd by model at the run grain",
    async () => {
      await store.insertBatch(sampleEvents());
      const query: CompiledQuery = {
        source: "runs",
  ...
  it(
    "computes ratio (error rate) by toolName",
    async () => {
      await store.insertBatch(sampleEvents());
      const query: CompiledQuery = {
        source: "events",
  ...
  it(
    "buckets a count over time (day grain)",
    async () => {
      await store.insertBatch(sampleEvents());
      const query: CompiledQuery = {
        source: "events",
  ...
  it(
    "getTrace returns detail with events ordered by step_index",
    async () => {
      await store.insertBatch(sampleEvents());
  ...
  it(
    "getTrace returns null for unknown trace",
    async () => {
      await store.insertBatch(sampleEvents());
